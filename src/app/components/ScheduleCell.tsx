...
const ScheduleCell: React.FC<ScheduleCellProps> = ({
    ...
    const updateLocalData = async (key: string, value: any) => {
        try {
            ...
            throw error instanceof SchedulerError
              ? error
              : new SchedulerError('Failed to update schedule', ErrorCodes.DATA_UPDATE_ERROR);
        }
    };
    ...
    const handleClearDay = async () => {
        if (currentData.id) {
            try {
                ...
                throw error instanceof SchedulerError
                    ? error
                    : new SchedulerError('Failed to clear day', ErrorCodes.DATA_UPDATE_ERROR);
            }
        }
  ...
  return (
    ...
    >
      <div
        className={`relative border border-slate-700 rounded-md min-h-[200px] group ${groupColor} ${
      ...
      >
        ...
        <div className="space-y-2 p-2">
          {isLocked && !isHeadSetter && (
            <div className="text-amber-500 text-sm mb-2">This cell is being edited by another user</div>
          )}
            ...
            <>
              <div className="flex items-center gap-2">
                <MultiSelect<Wall>
                  ...
                  getId={getWallId}
                  groupBy={(wall) =>
                    wall.wall_type === 'boulder' ? 'Boulder Walls' : 'Rope Walls'
                  }
          ...
          <textarea
            rows={2}
              style={{ minHeight: '2rem', overflowY: 'auto' }}
            className="w-full bg-slate-800 text-slate-200 border border-slate-700 rounded-md p-2 resize-none"
            value={localComment}
            onChange={(e) => {
        ...
        <DialogContent className="bg-slate-800 border-slate-700">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">
              Are you sure you want to clear all schedule entries for {getGymName(gym)} on{' '}
              {formatDate(date)}? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
...
const calculateMetrics = (
    ...
    const totalClimbsPerSetter = selectedWalls.reduce(
        (sum, wall) => sum + wall.climbs_per_setter,
        0
    );
    const wallCount = selectedWalls.length;

    const avgDifficulty =
        wallCount > 0 ? Number((totalDifficulty / wallCount).toFixed(1)) : 0;
