...
import { getDateForDatabase, createStandardizedDate } from '../utils/dateUtils';
import { 
  DESIGN_GYM_CONFIG,
  DENTON_GYM_CONFIG,
  HILL_GYM_CONFIG,
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-4">
      <Card className="bg-slate-800 border-slate-700">
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 overflow-auto">
            {dateRange.map(date => {
              const dateStr = getDateForDatabase(date);
              const dayEntries = scheduleData.filter(entry => 
                entry.schedule_date === dateStr
              );
